...
class SearchLocaitonViewController: UIViewController, ViewModelBindableType {
    ...
    func bindViewModel() {
        viewModel.adderData
            .bind(to: adderCollectionView.rx.items(cellIdentifier: String(describing: AdderCollectionCell.self), cellType: AdderCollectionCell.self)) { (_, element, cell) in
        ...
        Observable
            ...
            .bind { [unowned self] _, element in
                if let adder = element as? Address {
...
@available(iOS 13, *)
struct SearchLocaitonVCPreview: PreviewProvider {
    static var devices = ["iPhone 11 Pro", "iPhone 12 Pro Max"]
    static var previews: some View {
        ForEach(devices, id: \.self) { deviceName in
            // view controller using programmatic UI
            SearchLocaitonViewController().toPreview().previewDevice(PreviewDevice(rawValue: deviceName))
